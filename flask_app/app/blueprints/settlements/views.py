...
from app.models.local import InsuranceLocal, SettlementLocal 
...
def make_settlement():
    ...
    if not current_user.is_anonymous:
        ...
        user_insurances = InsuranceLocal.query.all()
    ...
    if form.validate_on_submit():
        if not current_user.is_anonymous:
            ...
            new_settlement = SettlementLocal(
                insurance_id=form.insurance_label.data,
                description=form.description.data,
                sum=form.sum.data
            )
...
def settlement_list():
    if current_user.is_anonymous:
        settlements = db.session.query(SettlementLocal)\
            .join(SettlementLocal.insurance)\
            .order_by(asc(SettlementLocal.id)).all()
    else:
